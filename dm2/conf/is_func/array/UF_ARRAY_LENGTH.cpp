...
/**
* 引数1の配列の引数2で指定された次元でのサイズを取得する
* @author    Nagoya University
* @date      2020/03/16
* @param     引数1=vector(double), 引数2=int
* @return    long
*/
extern "C" vector< vector<string> > UF_ARRAY_LENGTH(vector< vector<string> > argList)
{
  ...
  for(int i=0; i<argList.size(); i++)
  {
    ...
    vector< vector<int> > dimenList; // 次元毎の配列サイズ格納用vector
    bool check = true;
    ...
    // 次元毎の配列数をVectorに格納
    createDimenList(a1, &dimenList, &dimenStrList);
    ...
    // 次元チェックを行い正常に終了した場合は
    // リターン値を取得する
    // Vectorの範囲外の場合、Exceptionをthrowして
    // 次元の範囲外であることを通知する
    if (check) {
      int retCount;
      try {
        retCount = dimenList.at(a2-1).at(0);
        ret.push_back(std::to_string(retCount));
      } catch (out_of_range) {
        throw CastException("Array dimension is exceeded!");
      }
    }
