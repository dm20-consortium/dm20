...
* @date      2020/03/09
* @param     引数1=geometry, 引数2=int
* @return    geometry
*/
extern "C" vector<vector<string>> UF_GCS_TO_JGD2011(vector<vector<string>> argList)
{
  ...
  //printf("UF_GCS_TO_JGD2011 called!\n");
  ...
  for(int i=0; i<argList.size(); i++)
  {
    ...
    // 地理座標のSRIDか確認する
    if (srId != JGD2011_SRID_GEO) {
      throw CastException("SRID error: " + to_string(srId));
    }

    // JGD2011のSRIDか確認する
    if ((a2 < JGD2011_SRID_XY_S) || (a2 > JGD2011_SRID_XY_E)) {
      if (a2 != JGD2011_SRID_GEO) {
        throw CastException("SRID error: " + to_string(srId));
      }
    }
    ...
    try {
      ...
      stringUtil.replace(q, "$3", to_string(a2));
