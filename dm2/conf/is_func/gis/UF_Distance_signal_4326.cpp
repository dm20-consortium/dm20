...
* @return    double
*/
extern "C" vector< vector<string> > UF_DISTANCE_SIGNAL_4326(vector< vector<string> > argList)
{
  vector< vector<string> > retList; // ISに返却する結果2次元配列(retList[row][col])
  IS::StringUtil stringUtil;
  std::map<std::string, double> crp2lat;
  std::map<std::string, double> crp2lon;
  //ガン研
  crp2lat["790529"] = 35.900697;
  crp2lon["790529"] = 139.938992;
  //科警研
  crp2lat["798721"] = 35.896065;
  crp2lon["798721"] = 139.941859;
  //若柴
  crp2lat["2"] = 35.897912;
  crp2lon["2"] = 139.949387;
  // 入力された各行に対して操作を行う
  for(int i=0; i<argList.size(); i++)
  {
    ...
    if (crp2lon.find(a3) == crp2lon.end()) continue;
    lon2 = crp2lon[a3];
    lat2 = crp2lat[a3];
    if (a1.length() <= 7 || a2.length() <= 7) {
      continue;
    }
