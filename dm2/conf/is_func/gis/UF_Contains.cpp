...
/**
* 引数1のGeometry内に引数2のGeometryが内包されているかを返却する。
* @author    Nagoya University
* @date      2020/03/13
* @param     引数1=geometry, 引数2=geometry
* @return    boolean(t=内包されている,f=内包されていない)
*/
extern "C" vector< vector<string> > UF_CONTAINS(vector< vector<string> > argList)
{
  //printf("UF_Contains called!\n");
  ...
  for(int i=0; i<argList.size(); i++)
  {
    ...
    // 内包されているかを取得
    int result = GEOSContains(g1,g2);
    if (result == 2) {
       throw CastException("GEOS ERROR. GEOSContains failed.");
    }
