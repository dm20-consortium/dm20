...
* @return    double
*/
extern "C" vector< vector<string> > UF_YMAX(vector< vector<string> > argList)
{
  //printf("UF_YMAX called!\n");
  ...
  for(int i=0; i<argList.size(); i++)
  {
    ...
    // WKTから指定次元のリストを取得
    vector<double> getNumList = GeoAccessor(a1,2);

    // 最大値を取得
    double max = *max_element(getNumList.begin(), getNumList.end());

    // 表示形式に整形
    string retString = to_string(max);
