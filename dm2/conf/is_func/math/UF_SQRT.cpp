...
* @return    double
*/
extern "C" vector< vector<string> > UF_SQRT(vector< vector<string> > argList)
{
  ...
  for(int i=0; i<argList.size(); i++)
  {
    ...
    // 入力チェック
    if (a1 < 0) {
      throw CastException("cannot take square root of a negative number!");
    }

    // 平方根を求める
    double retDouble = sqrt(a1);
