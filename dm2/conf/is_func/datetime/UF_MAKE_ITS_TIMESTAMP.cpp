...
* @return    long
*/
extern "C" vector< vector<string> > UF_MAKE_ITS_TIMESTAMP(vector< vector<string> > argList)
{
  ...
  {
    vector<string> ret;
    for(int j=0; j<argList.at(i).size(); j++){
      if(std::stod(argList.at(i).at(j)) < 0){
        throw CastException("Data type is not match!");
      }
    }
    a1 = std::stod(argList.at(i).at(0)); // 引数1
    a2 = std::stod(argList.at(i).at(1)); // 引数2
    a3 = std::stod(argList.at(i).at(2)); // 引数3
    a4 = std::stod(argList.at(i).at(3)); // 引数4
    a5 = std::stod(argList.at(i).at(4)); // 引数5
    a6 = std::stod(argList.at(i).at(5)); // 引数6
    a7 = std::stod(argList.at(i).at(6)); // 引数7
    itsTime = date2ut(a1,a2,a3,a4,a5,a6);
    itsTime *= 1000;
    itsTime += a7;
