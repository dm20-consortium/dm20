...
* `main` ブランチは安定版として管理されています。
* `feature/*` などの開発ブランチには、開発途中の機能、実験的な変更、一時的なビルド不具合や動作不具合が含まれる場合があります。
* 新機能は開発ブランチで実装・検証された後に `main` ブランチへマージされます。
* 特定機能の評価や開発への参加を目的としない場合は、`main` ブランチの利用を推奨します。
