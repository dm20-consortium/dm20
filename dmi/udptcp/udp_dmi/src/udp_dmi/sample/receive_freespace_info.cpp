...
void Print(const DataClassesWithHeader<FreespaceInfo>& dataclass) {
...
int main(int argc, char** argv) {
  gflags::SetUsageMessage(
      "A sample that receive freespace info UDP datagram");
  gflags::ParseCommandLineFlags(&argc, &argv, true);

  SocketReceiver<FreespaceInfo, DataClassesWithHeader>::SharedPtr recv(new SocketReceiver<FreespaceInfo, DataClassesWithHeader>(FLAGS_receive_port,
