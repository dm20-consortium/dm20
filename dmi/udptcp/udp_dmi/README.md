...
`build_udp_dmi.bash` の中身は、下記の通り、必要なライブラリである `cool4_api_dataclass`, `dmi_utils` と、`udp_dmi` 本体のビルド・インストールをしています。

```bash
set -e 
for dir in cool4_api_dataclass dmi_utils udp_dmi; do 
  cd ${dir} 
